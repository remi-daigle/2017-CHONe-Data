...
Lastly, there are some column names that are not 'up to code', so to avoid a stern talking to from CHONe's data manager, let's fix that now!

```{r}
names(larvalAbundance)

names(larvalAbundance)[names(larvalAbundance)=="long"] <- "decimalLongitude"
names(larvalAbundance)[names(larvalAbundance)=="lat"] <- "decimalLatitude"
