...
Let's begin by downloading a document that all of us should be aware of, *i.e.* Paul Snelgrove's entitled 'Discoveries of the Census of Marine Life:
Making Ocean Life Count' (text available for free [here](http://www.cambridge.org/download_file/153663)).
...
<br/>
As you can see, the resulting text is coerced as a page per string for a total of 398 pages, with lines divided by '\\n'. Let's tidy up the text to make it more easily usable for further analyses.

## Extracting tables from pdfs in R

<br/>
If you look at the actual pdf, you will realize that tables are not imported in r with the `pdf_text` function. If you wish to do so, there is a package called [`tabulizer`](https://ropensci.org/blog/blog/2017/04/18/tabulizer).
...
```{r, tidy text}
    ...
    # Transform as a matrix
        ...
        for(i in 1:length(book)) {
            bk <- cbind(book[[i]], rep(i, length(book[[i]])), 1)
    ...
    # Convert to data.frame
        ...
        bookMat[, 'document'] <- as.numeric(paste(bookMat[, 'document']))
        kable(bookMat[1:10, ])
```
...
```{r, words}
    ...
    # Total number of words
        totalWords <- bookWords %>% summarize(total = sum(n))
        totalWords
```
...
```{r, words chapter}
    # Count per word per chapter
        bookWordsChapter <- bookChapter %>%
                                unnest_tokens(word, text) %>%
                                count(chapter, word, sort = TRUE) %>%
                                ungroup()

    # Attach total number of words per chapter                            
        totalWordsChapter <- bookWordsChapter %>% group_by(chapter) %>% summarize(total = sum(n))
        totalWordsChapter

        bookWordsChapter <- left_join(bookWordsChapter, totalWordsChapter)
        bookWordsChapter

        ggplot(bookWordsChapter, aes(n/total, fill = chapter)) +
...
```{r, stop words}
    # Count per word after stop words removal
        ...
        totalWords <- bookWords %>% group_by(document) %>% summarize(total = sum(n))
        bookWords <- left_join(bookWords, totalWords)
        bookWords

        ggplot(bookWords, aes(n/total, fill = document)) +
          geom_histogram(show.legend = FALSE) +
          xlim(NA, 0.0009) +
        ...
        # plot
            ggplot(paulsentiment, aes(index, sentiment, fill = document)) +
                    ...
                    labs(title = "Sentiment in Paul Snelgrove's writing",
                         y = "Sentiment") +
                    scale_fill_viridis(end = 0.75, discrete=FALSE, direction = -1) +
...
paulSentences <- tidyBook %>%
                    group_by(document) %>%
                    unnest_tokens(sentence, text, token = "sentences") %>%
                    ungroup()


paulSentences$sentence[50]
```
...
wordOcc %>%
        ...
        netwD3 <- networkD3::igraph_to_networkD3(x, group = rep(1, vcount(x)), what = 'both')
...
```{r, wordcloud}
        ...
        bookWords <- as.data.frame(bookWords[, c('word','n')])

        # letterCloud(bookWords, word = 'Paul Snelgrove',size = 2, color="white", backgroundColor=1)

wordcloud2(bookWords, color = "random-light", backgroundColor = "grey")
...
wordcloud2(dat, figPath = "./img/butterfly.png", size = 2, color="white", backgroundColor=1)
