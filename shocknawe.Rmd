...
library(rmarkdown)
render(input = 'shocknawe.rmd', 'all')
...
pnt <- SpatialPoints(coords = data.frame(x = 174.764474, y = -36.877245), proj4string = CRS('+init=epsg:4326'))
...
With that in mind, here is a list (not ordered with any sort of priority or relevance) of a few resources from which we are regularly using to keep on top of new developments and to search for help achieving whatever it is we imagine we could do. Use it abundantly!
...
mapview::mapview(OBISpt, cex = 4)

leaflet::leaflet() %>%
  leaflet::addTiles() %>%
  leaflet::addCircleMarkers(data = OBISpt, popup = popupTable(OBISpt, zcol = c("species", "yearcollected")))
```
